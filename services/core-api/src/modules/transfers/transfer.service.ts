...
import { SlowLog, timedOp } from '../../observability/metrics';
...
export interface TransferInput {
  ...
  description?: string;
  traceId?: string;
  log?: SlowLog; // logger de la petición, para registrar pasos SQL lentos o fallidos
}
...
async function lockAccounts(
  ...
  from: string,
  to: string,
  ctx: { log?: SlowLog; traceId?: string },
  ...
  if (config.lockOrdering) {
    const res = await timedOp('lock_accounts', () => c.query<LockedAccount>(`${sql} ORDER BY id FOR UPDATE`, [[from, to]]), ctx);
    rows.push(...res.rows);
  } else {
    const first = await timedOp('lock_from', () => c.query<LockedAccount>(`${sql} FOR UPDATE`, [[from]]), ctx);
    if (config.simulatedLockDelayMs > 0) await sleep(config.simulatedLockDelayMs);
    const second = await timedOp('lock_to', () => c.query<LockedAccount>(`${sql} FOR UPDATE`, [[to]]), ctx);
...
export async function executeTransfer(input: TransferInput): Promise<TransferResult> {
  ...
  const ctx = { log: input.log, traceId: input.traceId };
  const outcome = await withTransaction(async (c) => {
    // 1) Bloquear ambas cuentas (orden determinista) y validar.
    const accounts = await lockAccounts(c, input.fromAccount, input.toAccount, ctx);
    ...
    const inserted = await timedOp('insert_tx', () => c.query(
    ...
    const debit = await timedOp('debit', () => c.query(
      `UPDATE accounts
    ...
    const credit = await timedOp('credit', () => c.query(
      `UPDATE accounts
          SET balance = balance + $1::numeric, version = version + 1, updated_at = now()
        WHERE id = $2
        RETURNING balance::text AS balance`,
    ...
    await timedOp('outbox', () => c.query(
      ...
      [
        ...
        JSON.stringify({
          ...
          // Contexto W3C de la traza: el relay y el worker lo continúan, así la traza cruza el outbox y el stream.
